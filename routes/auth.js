...
// CUST LOGIN ROUTE
router.post("/login", [helper.hasAuthFields], async (req, res) => {
  ...
  let token = jwt.sign(
    { state: "true", mobile: req.body.mobile },
    helper.secret,
    {
      algorithm: "HS512",
      expiresIn: "1h",
...
router.post(
  "/tnr/login",
  [helper.hasAuthFields, helper.isPasswordAndMobileMatchTrainer],
  async (req, res) => {
    const myMobile = req.body.mobile;
  ...
  [
    ...
    body("Data.LIMobile").custom((value) => {
      return helper.database
        .table("customer")
        .filter({ mobile: { $in: [value] } })
        .get()
        .then((user) => {
          if (user) {
            console.log(user);
            return Promise.reject(
              "Mobile number already exists, choose another one."
            );
          }
        });
    }),
